...
# roomGraph={0: [(3, 5), {'n': 1, 'e': 5}], 1: [(3, 6), {'s': 0, 'n': 2}], 2: [(3, 7), {'s': 1}], 3: [(4, 7), {'w': 2, 's': 4}], 4: [(4, 6), {'n': 3, 's': 5}], 5: [(4, 5), {'n': 4, 'w': 0}]}
...
rooms = {}
visited = set()
def to_dead_end(current_room, prev_direction = None, prev_room_id = None):
    ...
    if prev_direction:
        rooms[current_room.id][opposites[prev_direction]] = prev_room_id
    for next_room in rooms[current_room.id]:
        if rooms[current_room.id][next_room] == '?':
            ...
            to_dead_end(player.currentRoom, next_room, current_room.id)
            return




def bfs(current_room_id):
    queue = Queue()
...
def traverse():
    while len(roomGraph) != len(visited):
        to_dead_end(player.currentRoom)
